/**
 * SpendWise — Mock Data / Service Layer
 * ---------------------------------------------------
...
const SpendWiseAPI = (() => {
  ...
  const financialProfile = {
    ...
    existingEmi: 4000,
    commitments: [
      { id: "c_1", name: "Bike Loan", monthlyAmount: 2800, remainingMonths: 9 },
      { id: "c_2", name: "Streaming subscriptions", monthlyAmount: 1200, remainingMonths: null },
    ],
    preferences: {
  ...
  const decisions = [
    {
      purchase: {
        id: "p_001", name: "MacBook Air M4", category: "Electronics",
        price: 89999, purchaseType: "ONE_TIME",
        reason: "Current laptop is becoming too slow for development work.",
        createdAt: "2026-08-27",
      },
      analysis: {
        purchaseId: "p_001",
        decision: "WAIT",
        affordability: "CONDITIONALLY_AFFORDABLE",
        financialImpact: "MEDIUM",
        goalImpact: "HIGH",
        estimatedAffordableDate: "2027-03-15",
        goalDelayMonths: 3,
        monthlySurplusBefore: 28000,
        monthlySurplusAfterPurchase: 22800,
        goalCompletionBefore: "2027-01-15",
        goalCompletionAfter: "2027-04-15",
        reasons: [
          { type: "positive", text: "Your monthly cash flow can comfortably support the purchase." },
          { type: "warning", text: "Your savings buffer would fall well below your comfort threshold." },
          { type: "warning", text: "Your Emergency Fund goal would be delayed by roughly 3 months." },
          { type: "positive", text: "Waiting 3 months and saving the surplus removes almost all of the delay." },
        ],
        alternatives: [
          { name: "Lenovo IdeaPad Slim 5", price: 65000, savings: 24999, impact: "LOWER" },
          { name: "Refurbished MacBook Air M2", price: 58000, savings: 31999, impact: "LOWER" },
        ],
        actionPlan: [
          "Save an additional ₹7,000 per month toward this purchase.",
          "Maintain at least ₹60,000 in your emergency buffer at all times.",
          "Re-evaluate this decision after 3 months.",
          "Watch for a discount that brings the price below ₹80,000.",
        ],
      },
    },
    {
      purchase: {
        id: "p_002", name: "Gym Membership (Annual)", category: "Health & Fitness",
        price: 12000, purchaseType: "ONE_TIME",
        reason: "Want to commit to a consistent fitness routine.",
      ...
      analysis: {
        ...
        financialImpact: "LOW",
        goalImpact: "LOW",
        estimatedAffordableDate: "2026-08-24",
        goalDelayMonths: 0,
        monthlySurplusBefore: 28000,
        monthlySurplusAfterPurchase: 27000,
        goalCompletionBefore: "2027-01-15",
        goalCompletionAfter: "2027-01-22",
        reasons: [
          { type: "positive", text: "This fits comfortably within your monthly surplus." },
          { type: "positive", text: "Your emergency fund timeline barely shifts — about one week." },
          { type: "positive", text: "Recurring cost is well within your subscription budget." },
        ],
        alternatives: [
          { name: "Month-to-month membership", price: 1500, savings: 0, impact: "SIMILAR" },
        ],
        actionPlan: [
          "Proceed with the purchase — no changes to your plan required.",
          "Track this as a recurring annual commitment for next year's renewal.",
        ],
      },
    },
    {
      purchase: {
        id: "p_003", name: "Sony WH-1000XM5 Headphones", category: "Electronics",
        price: 18500, purchaseType: "ONE_TIME",
        reason: "Better focus while working from noisy cafes.",
      ...
      analysis: {
        ...
        financialImpact: "LOW",
        goalImpact: "MEDIUM",
        estimatedAffordableDate: "2026-08-21",
        goalDelayMonths: 1,
        monthlySurplusBefore: 28000,
        monthlySurplusAfterPurchase: 26200,
        goalCompletionBefore: "2027-01-15",
        goalCompletionAfter: "2027-02-15",
        reasons: [
          { type: "warning", text: "You purchased a similar audio accessory two months ago." },
          { type: "warning", text: "This category has the highest discretionary spend in your profile." },
          { type: "positive", text: "A lower-cost alternative meets the same underlying need." },
        ],
        alternatives: [
          { name: "Boat Rockerz 550", price: 2200, savings: 16300, impact: "MUCH LOWER" },
        ],
        actionPlan: [
          "Skip this purchase for now — the need is already served by your current headphones.",
          "Revisit only if your current pair fails or your role changes to more calls.",
        ],
      },
    },
  ];
  ...
  const plannedPurchases = [
    { id: "pp_001", name: "Laptop", price: 85000, targetDate: "2027-03-01", progress: 62 },
    { id: "pp_002", name: "New Phone", price: 45000, targetDate: "2026-12-01", progress: 84 },
  ];
  ...
  return {
    ...
    // GET /api/decisions/{id}  (purchase id used as decision id here)
    async getDecision(id) {
      await delay(80);
      return JSON.parse(JSON.stringify(decisions.find((d) => d.purchase.id === id)));
    },

    // GET /api/purchases/planned
    async getPlannedPurchases() {
      await delay(80);
      return JSON.parse(JSON.stringify(plannedPurchases));
    },

    // POST /api/purchases  ->  POST /api/purchases/{id}/analyze
    // In this prototype the "decision engine" runs client-side as a stand-in
    // for a Spring Boot service. The shape of the response matches exactly
    // what that service will eventually return.
...
const DecisionEngineMock = (() => {
  function run(purchase, profile, goalsList) {
    const goal = goalsList.find((g) => g.id === profile.savingsGoalId) || goalsList[0];
  ...
  function simulate(purchase, profile, goal, { waitMonths = 0, extraSaving = 0, discount = 0 }) {
    ...
    const goalDelayMonths = Math.max(Math.round(impactedMonths - baselineMonths), 0);

    const bufferRatio = savingsAfterPurchase / profile.currentSavings;
    let decision = "BUY";
...
function sidebarMarkup(activePath){
  ...
  return `
    ...
    <a class="profile-mini" href="#/profile">
      ...
      <div>
        <div class="pname">Chetan</div>
        <div class="pemail">chetan@example.com</div>
      </div>
    </a>
  `;
}
...
(async function initDashboard(){
  ...
  document.getElementById('greeting').textContent = `Good morning, ${user.name} 👋`;
...
function renderSnapshot(profile){
  ...
  const cards = [
    { title: 'Monthly Income', value: profile.monthlyIncome, icon: ICON.income, indicator: 'Stable this month', tone: 'flat' },
    { title: 'Monthly Expenses', value: profile.monthlyExpenses, icon: ICON.expense, indicator: '53% of income', tone: 'flat' },
    { title: 'Available Monthly Surplus', value: surplus, icon: ICON.surplus, indicator: `${Math.round(surplus/profile.monthlyIncome*100)}% savings rate`, tone: 'up' },
    { title: 'Current Savings', value: profile.currentSavings, icon: ICON.savings, indicator: '53% toward Emergency Fund', tone: 'up' },
  ];
  ...
  el.innerHTML = `
    ...
    <div class="health-top mt-16">
      <div class="health-dial-wrap">
        ${decisionDial({ value: user.financialHealthScore, color: 'var(--buy)', size: 170 })}
        <div class="health-score-label">
          <div class="n">${user.financialHealthScore}<span style="font-size:16px;color:var(--ink-faint);">/100</span></div>
          <div class="l">${user.financialHealthLabel}</div>
        </div>
      </div>
      <p class="health-desc">Your current spending pattern supports your savings goal, but large purchases should be planned carefully to avoid setting your Emergency Fund back.</p>
    </div>
    <div class="health-metrics">
      ...
      <div class="health-metric"><span class="m-label">Goal Progress</span><span class="m-value">53%</span></div>
...
function renderGoal(goal){
  ...
  el.innerHTML = `
    ...
    <a href="goals.html" class="btn btn-secondary btn-block mt-16">View Goal</a>
  `;
}
...
function renderDecisions(decisions){
  ...
  el.innerHTML = decisions.map(d => `
    <div class="decision-row" onclick="window.location.href='decisions.html?id=${d.purchase.id}'">
...
(function(){
  ...
  document.getElementById('purchase-form').addEventListener('submit', async (e) => {
    ...
    currentGoal = goals.find(g => g.id === currentProfile.savingsGoalId) || goals[0];
  ...
  function renderRecap(profile){
    ...
    const rows = [
      ...
      ['Savings goal', Fmt.currency(currentGoal.targetAmount)],
      ...
      <div class="card mt-24 fade-up">
        ...
        <div class="impact-compare">
          <div class="impact-col before">
            ...
            <div class="impact-stat"><div class="is-label">Goal completion</div><div class="is-value">${Fmt.compactMonth(goal.targetDate)}</div></div>
          </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (async function(){
    ...
    grid.innerHTML = goals.map(g => {
      ...
      <div class="create-goal-card" onclick="alert('This would open a goal-creation form connected to POST /api/goals.')">
        <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 5v14M5 12h14"/></svg>
        <span style="font-weight:600;">Create Goal</span>
      </div>
    `;
  })();
  ...
  (async function(){
    ...
    document.getElementById('commitments-list').innerHTML = profileState.commitments.map(c => `
  ...
  document.getElementById('save-btn').addEventListener('click', async () => {
    ...
    await SpendWiseAPI.updateFinancialProfile(patch);
