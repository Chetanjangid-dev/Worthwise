 ...
 * element is emptied AND hidden (display:none) and the mobile topbar     savingsGoalId: "",
    existingEmi: 0,
    emergencyFundTarget: 0,
    commitments: [],
    preferences: {
...
// Mobile primary navigation — rendered as a fixed bottom bar on phones.
const BOTTOM_NAV = [
  { route: '#/dashboard', label: 'Overview',  icon: 'grid' },
  { route: '#/analyze',   label: 'Analyze',   icon: 'scan' },
  { route: '#/decisions', label: 'Decisions', icon: 'list' },
  { route: '#/goals',     label: 'Goals',     icon: 'target' },
  { route: '#/profile',   label: 'Profile',   icon: 'sliders' },
];

function initialsFrom(user){
  const src = ((user && (user.name || user.email)) || '').trim();
  if (!src) return 'U';
...
function sidebarMarkup(activePath){
  ...
  return `
    ...
    <a class="profile-mini" href="#/profile">
      <div class="avatar">${initialsFrom(currentUserCache)}</div>
      <div>
        <div class="pname">${currentUserCache.name || 'User'}</div>
        <div class="pemail">${currentUserCache.email || ''}</div>
      </div>
...
/**
 * Mobile chrome: a slim branded topbar plus a fixed bottom navigation bar.
 * Logged-out users get only the logo — no nav to open, nothing to see —
 * until they actually authenticate. This does not touch any backend call.
 */
function mobileTopbarMarkup(authed, activeRoute){
  if (!authed) {
    return `<div class="mobile-topbar">${logoMarkup()}</div>`;
  }
  const items = BOTTOM_NAV.map(item => `
    <a href="${item.route}" class="${item.route === activeRoute ? 'active' : ''}">
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">${ICONS[item.icon]}</svg>
      <span>${item.label}</span>
    </a>`).join('');
  return `
    <div class="mobile-topbar">${logoMarkup()}</div>
    <nav class="bottom-n    savingsGoalId: "",
    existingEmi: 0,
    emergencyFundTarget: 0,
    commitments: [],
    preferences: {
  ...
  return {
    ...
    // Logout: clears the session token and returns the user to the public
    // landing page. Because isAuthenticated() reads the same localStorage
    // key, the very next render sees the logged-out state and hides every
    // piece of authenticated UI. Full reload guarantees no stale DOM.
    logout(){
      authToken = '';
      localStorage.removeItem('spendwise_token');
...
/**
 * Renders the authenticated sidebar. Logout now lives inside a profile
 * dropdown at the bottom (see initShell() for the open/close wiring).
 */
function sidebarMarkup(activePath){
  const activeRoute = '#/' + activePath.replace('.html','');
  const items = NAV_ITEMS.map(item => {
  ...
  const name = (currentUserCache.name || 'User').trim();
  ...
  return `
    ...
    <div class="profile-wrap">
      <button class="profile-mini" id="profile-mini" type="button"
      ...
      <div class="profile-menu" id="profile-menu" role="menu" hidden>
        ...
        <a href="#/profile" data-scroll-target="settings-card" role="menuitem">Settings</a>
        <div class="menu-sep"></div>
        <button type="button" class="menu-danger" id="logout-btn" role="menuitem">Logout</button>
      </div>
    </div>
  `;
}
 ...
 *
 * FIX (issue 3b): the profile dropdown (My Profile / Settings / Logout)
 * is wired here. Because initShell() is re-run on every navigation, the
 * listeners are always bound to the current DOM node — no stale handlers.
 */
function initShell(){
  ...
  if (mini && menu) {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (menu.contains(e.target) || mini.contains(e.target)) return;
      setOpen(false);
    });
...
function navigate(){
  ...
  if (appViews) appViews.style.display = '';
  ...
  // FIX (issue 3): initShell() now self-guards on auth state (see initShell
  // above), so it's safe — and necessary — to call it on every route,
  // including '#/auth'. Previously this call was skipped for the auth route,
  // which meant a stale, fully-populated sidebar (rendered by the
  // unconditional DOMContentLoaded -> initShell() call) could be left
  // sitting in the DOM behind the login screen.
  initShell();
