...
/**
 * Bottom tab bar shown on mobile (<=900px) instead of the sidebar drawer.
 * Reuses NAV_ITEMS but drops the Settings entry (still reachable from
 * Profile) so five icons fit comfortably in a thumb-friendly row.
 */
function bottomNavMarkup(activePath){
  const activeRoute = '#/' + activePath.replace('.html','');
  const items = NAV_ITEMS.filter(item => !item.scrollTarget);
  return items.map(item => {
    const isActive = activeRoute === item.route;
    return `<a class="bottom-nav-link${isActive ? ' active' : ''}" href="${item.route}">
      <svg width="21" height="21" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">${ICONS[item.icon]}</svg>
      <span>${item.label === 'Analyze Purchase' ? 'Analyze' : item.label === 'Financial Profile' ? 'Profile' : item.label === 'My Decisions' ? 'Decisions' : item.label}</span>
    </a>`;
  }).join('');
}
...
function initShell(){
  ...
  if (!authed) {
    ...
    if (bottomNav) { bottomNav.innerHTML = ''; bottomNav.style.display = 'none'; }
    return;
  }
  ...
  if (bottomNav) {
    ...
    bottomNav.innerHTML = bottomNavMarkup(path);
  }
}
